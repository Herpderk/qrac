...
class NatNetClient:
    ...
    def __init__(
        ...
        self.server_ip_address = server_addr

        # Change this value to the IP address of your local network interface
        self.local_ip_address = local_addr

        # This should match the multicast address listed in Motive's streaming settings.
        self.multicast_address = multicast_addr
        ...
        self.rigid_body_listener = rigid_body_cb
        self.new_frame_listener  = new_frame_cb
    ...
    def __unpack_rigid_body( self, data, major, minor, rb_num):
        ...
        rigid_body = qrac.optitrack.MoCapData.RigidBody(new_id, pos, rot)
        ...
        if( major < 3  and major != 0) :
            ...
            for i in marker_count_range:
                rb_marker_list.append(qrac.optitrack.MoCapData.RigidBodyMarker())
    ...
    def __unpack_skeleton( self, data, major, minor, skeleton_num=0):
        ...
        skeleton = qrac.optitrack.MoCapData.Skeleton(new_id)
    ...
    def __unpack_asset( self, data, major, minor, asset_num=0):
        ...
        offset += 4
        asset = qrac.optitrack.MoCapData.Asset()
...
#Unpack Mocap Data Functions
    def __unpack_frame_prefix_data( self, data):
        ...
        frame_prefix_data=qrac.optitrack.MoCapData.FramePrefixData(frame_number)
    ...
    def __unpack_legacy_other_markers( self, data, packet_size, major, minor):
        ...
        other_marker_data = qrac.optitrack.MoCapData.LegacyMarkerData()
    ...
    def __unpack_marker_set_data( self, data, packet_size, major, minor):
        marker_set_data=qrac.optitrack.MoCapData.MarkerSetData()
        offset = 0
        # Markerset count (4 bytes)
        ...
        for i in range( 0, marker_set_count ):
            marker_data = qrac.optitrack.MoCapData.MarkerData()
            # Model name
    ...
    def __unpack_rigid_body_data( self, data, packet_size, major, minor):
        rigid_body_data = qrac.optitrack.MoCapData.RigidBodyData()
        offset = 0
    ...
    def __unpack_skeleton_data( self, data, packet_size, major, minor):
        skeleton_data = qrac.optitrack.MoCapData.SkeletonData()
    ...
    def __unpack_labeled_marker_data( self, data, packet_size, major, minor):
        labeled_marker_data = qrac.optitrack.MoCapData.LabeledMarkerData()
        offset = 0
        ...
        if( ( major == 2 and minor > 3 ) or major > 2 ):
            ...
            for lm_num in range( 0, labeled_marker_count ):
                ...
                labeled_marker = qrac.optitrack.MoCapData.LabeledMarker(tmp_id,pos,size,param, residual)
    ...
    def __unpack_force_plate_data( self, data, packet_size, major, minor):
        force_plate_data = qrac.optitrack.MoCapData.ForcePlateData()
        n_frames_show_max = 4
        offset = 0
        ...
        if( ( major == 2 and minor >= 9 ) or major > 2 ):
            ...
            for i in range( 0, force_plate_count ):
                ...
                offset += 4
                force_plate = qrac.optitrack.MoCapData.ForcePlate(force_plate_id)
                ...
                for j in range( force_plate_channel_count ):
                    fp_channel_data = qrac.optitrack.MoCapData.ForcePlateChannelData()
    ...
    def __unpack_device_data( self, data, packet_size, major, minor):
        device_data = qrac.optitrack.MoCapData.DeviceData()
        n_frames_show_max = 4
        offset = 0
        ...
        if ( major == 2 and minor >= 11 ) or (major > 2) :
            ...
            for i in range( 0, device_count ):
                ...
                offset += 4
                device = qrac.optitrack.MoCapData.Device(device_id)
                # Channel Count
                ...
                for j in range( 0, device_channel_count ):
                    device_channel_data = qrac.optitrack.MoCapData.DeviceChannelData()
    ...
    def __unpack_frame_suffix_data( self, data, packet_size, major, minor):
        frame_suffix_data = qrac.optitrack.MoCapData.FrameSuffixData()
        offset = 0
    ...
    def __unpack_mocap_data( self, data : bytes, packet_size, major, minor):
        mocap_data = qrac.optitrack.MoCapData.MoCapData()
    ...
    def __unpack_marker_set_description( self, data, major, minor):
        ms_desc = qrac.optitrack.DataDescriptions.MarkerSetDescription()
    ...
    def __unpack_rigid_body_description( self, data, major, minor):
        rb_desc=qrac.optitrack.DataDescriptions.RigidBodyDescription()
        offset = 0
        ...
        if (major >= 3) or (major == 0) :
            ...
            for marker in marker_count_range:
                ...
                rb_marker=qrac.optitrack.DataDescriptions.RBMarker(marker_name,active_label,marker_offset)
    ...
    def __unpack_skeleton_description( self, data, major, minor):
        skeleton_desc = qrac.optitrack.DataDescriptions.SkeletonDescription()
        offset = 0
    ...
    def __unpack_force_plate_description(self, data, major, minor):
        ...
        offset = 0
        if major >= 3:
            fp_desc = qrac.optitrack.DataDescriptions.ForcePlateDescription()
            # ID
            new_id = int.from_bytes( data[offset:offset+4], byteorder='little',  signed=True )
            offset += 4
    ...
    def __unpack_device_description(self, data, major, minor):
        ...
        if major >= 3:
            ...
            device_desc = qrac.optitrack.DataDescriptions.DeviceDescription(new_id,name,serial_number,device_type,channel_data_type)
    ...
    def __unpack_camera_description(self, data, major, minor):
        ...
        camera_desc=qrac.optitrack.DataDescriptions.CameraDescription(name, position, orientation)
    ...
    def __unpack_marker_description( self, data, major, minor ):
        ...
        marker_desc=qrac.optitrack.DataDescriptions.MarkerDescription(name, marker_id, initialPosition, marker_size, marker_params)
    ...
    def __unpack_asset_rigid_body_data( self, data, major, minor ):
        ...
        # Package for return object
        rigid_body_data=qrac.optitrack.MoCapData.AssetRigidBodyData(rbID, pos, rot, mean_error, marker_params)
    ...
    def __unpack_asset_marker_data( self, data, major, minor ):
        ...
        marker_data = qrac.optitrack.MoCapData.AssetMarkerData(marker_id, pos, marker_size, marker_params, residual)
        return offset, marker_data

    def __unpack_asset_data( self, data, packet_size, major, minor):
        asset_data = qrac.optitrack.MoCapData.AssetData()
    ...
    def __unpack_asset_description(self, data, major, minor):
        ...
        # package for output
        asset_desc = qrac.optitrack.DataDescriptions.AssetDescription(name, assetType, assetID, rigidbodyArray, markerArray)
    ...
    def __unpack_data_descriptions( self, data : bytes, packet_size, major, minor):
        data_descs = qrac.optitrack.DataDescriptions.DataDescriptions()
        offset = 0
